...
def main():
    ...
    df_dis_ingress = df_dis_ingress.toDF(
        *(c.replace(' ', '_') for c in df_dis_ingress.columns)
    )
    df_glo_ingress = df_glo_ingress.toDF(
        *(c.replace(' ', '_') for c in df_glo_ingress.columns)
    )
    df_ano_ingress = df_ano_ingress.toDF(
        *(c.replace(' ', '_') for c in df_ano_ingress.columns)
    )
    ## Remove parenthesises
    df_dis_ingress = df_dis_ingress.toDF(
        *(c.replace('(', '') for c in df_dis_ingress.columns)
    )
    df_dis_ingress = df_dis_ingress.toDF(
        *(c.replace(')', '') for c in df_dis_ingress.columns)
    )
    df_glo_ingress = df_glo_ingress.toDF(
        *(c.replace('(', '') for c in df_glo_ingress.columns)
    )
    df_glo_ingress = df_glo_ingress.toDF(
        *(c.replace(')', '') for c in df_glo_ingress.columns)
    )
    df_ano_ingress = df_ano_ingress.toDF(
        *(c.replace('(', '') for c in df_ano_ingress.columns)
    )
    df_ano_ingress = df_ano_ingress.toDF(
        *(c.replace(')', '') for c in df_ano_ingress.columns)
    )
    ...
    df_dis_ingress.write.format("delta").mode("overwrite").save(
        f"{s3_output_path}df_dis_bronze"
    )
    df_glo_ingress.write.format("delta").mode("overwrite").save(
        f"{s3_output_path}df_glo_bronze"
    )
    df_ano_ingress.write.format("delta").mode("overwrite").save(
        f"{s3_output_path}df_ano_bronze"
    )
...
if __name__ == "__main__":
    ...
    spark = (
        SparkSession.builder.appName("IngressToBronze")
